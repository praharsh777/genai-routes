...
export default function VisualizationSection({
  ...
  const depotCandidate =
    routes.flatMap((r) => r.stops).find((stop) => stop.name?.trim().toLowerCase() === "depot") ||
  ...
  const legendItems = routes.map((vehicle, idx) => {
    const distanceKm = ((vehicle.route?.routes?.[0]?.summary?.distance || 0) / 1000).toFixed(2);
    return {
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="p-4 border rounded-lg bg-muted/30 text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  Before: {(beforeMetrics.distance / 1000).toFixed(2)} km
                </p>
              ...
              <div className="p-4 border rounded-lg bg-muted/30 text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  Before: {(beforeMetrics.duration / 3600).toFixed(2)} hrs
                </p>
            ...
            <div className="lg:col-span-3">
              <div className="saas-card-lg overflow-hidden">
                <div className="p-6 border-b border-border bg-muted/30 flex items-center justify-between">
                  ...
                  >
                    <Zap className="w-3 h-3 mr-1" /> {routes.length > 0 ? "Optimized" : "Waiting"}
                  </Badge>
                </div>
